...
CodeMirror.defineSimpleMode('netlogo', {
  start: [
    {token: 'keyword', regex: /(?:\s|^)(?:to|to-report)(?:\s|$)/i, indent: true},
    {token: 'keyword', regex: /(?:\s|^)end(?:\s|$)/i, dedent: true},
    {token: 'keyword', regex: new RegExp("(?:\\s|^)(?:#{keywords.join('|')})(?:\\s|$)", 'i')},
    {token: 'keyword', regex: /[^\s\(\)\[\]]*-own(?:\s|$)/},
    {token: 'builtin', regex: new RegExp("\\s(?:#{commands.join('|')})(?:\\s|$)", 'i')},
    {token: 'variable-2', regex: new RegExp("\\s(?:#{reporters.join('|')})(?:\\s|$)", 'i')},
    {token: 'comment', regex: /;.*/}
    {token: 'string', regex: /"(?:[^\\]|\\.)*?"/},
